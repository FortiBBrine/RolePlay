...
public class CommandsMe implements CommandExecutor {
	private Core plugin;
	public CommandsMe(Core plugin) {
		this.plugin = plugin;
	}
	@Override
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		
		if (sender.hasPermission("roleplay.me")) {
			...
			for (Player pz : Bukkit.getOnlinePlayers()) {
				...
				if (x1 < x && x < x2 && y1 < y && y < y2 && z1 < z && z < z2) {
					pz.sendMessage(plugin.getConfig().getString("me").replace("{player}", p.getDisplayName()).replace("{message}", msg).replace("&", "\u00a7"));
				}
			}
			return true;
		}
