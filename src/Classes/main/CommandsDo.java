...
public class CommandsDo implements CommandExecutor {
	private Core plugin;
	public CommandsDo(Core plugin) {
		this.plugin = plugin;
	}
	@Override
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		
		if (sender.hasPermission("roleplay.do")) {
			...
			for (Player pz : Bukkit.getOnlinePlayers()) {
				...
				if (x1 < x && x < x2 && y1 < y && y < y2 && z1 < z && z < z2) {
					pz.sendMessage(plugin.getConfig().getString("do").replace("{player}", p.getDisplayName()).replace("{message}", msg).replace("&", "\u00a7"));
				}
			}
			return true;
		}
